...
void main() {
  group('PHEV Calculation Core — Стресс-тестирование и бизнес-логика', () {
    // ── Сценарий 1 ────────────────────────────────────────────────────────────
    test('test_first_entry_no_consumption: Первая заправка фиксирует базу, расход == null', () {
      final entries = [
        _makeFuelEntry(id: 1, odometer: 1000.0, volume: 45.0, isFullTank: true),
      ];

      final results = FuelEntryController.computeEntriesWithConsumption(entries);

      expect(results.length, equals(1));
      expect(results[0].consumption, isNull,
          reason: 'Первая заправка не имеет предыдущей точки отсчета, расход обязан быть null');
    });

    // ── Сценарий 2 ────────────────────────────────────────────────────────────
    test('test_ev_charging_isolation: 3 подряд зарядки батареи между заправками бензина', () {
      ...
      final entries = [
        ...
        _makeChargeEntry(id: 2, odometer: 1100.0, volume: 15.0, isFullTank: true),
        _makeChargeEntry(id: 3, odometer: 1200.0, volume: 14.0, isFullTank: true),
        _makeChargeEntry(id: 4, odometer: 1300.0, volume: 15.0, isFullTank: true),
      ...
      expect(fuel1.consumption, isNull, reason: 'Первая заправка бензина — база');
      ...
      expect(fuel2.consumption, closeTo(7.0, 0.001),
          reason: 'Бензиновая цепочка не должна учитывать одометры электрозарядок');
    ...
    // ── Сценарий 3 ────────────────────────────────────────────────────────────
    test('test_partial_fillup_accumulation: Классический алгоритм полного бака с буферизацией', () {
      ...
      final entries = [
        _makeFuelEntry(id: 1, odometer: 1000.0, volume: 45.0, isFullTank: true),
        _makeFuelEntry(id: 2, odometer: 1200.0, volume: 15.0, isFullTank: false),
        _makeFuelEntry(id: 3, odometer: 1350.0, volume: 10.0, isFullTank: false),
        _makeFuelEntry(id: 4, odometer: 1500.0, volume: 20.0, isFullTank: true),
      ];

      final results = FuelEntryController.computeEntriesWithConsumption(entries);

      expect(results[0].consumption, isNull, reason: 'Шаг 1: базовый полный бак');
      expect(results[1].consumption, isNull, reason: 'Шаг 2: дозаправка — расход null');
      expect(results[2].consumption, isNull, reason: 'Шаг 3: дозаправка — расход null');
      ...
      expect(results[3].consumption, closeTo(9.0, 0.001),
          reason: 'Расход на шаге 4 обязан аккумулировать все промежуточные дозаправки');
    });

    // ── Сценарий 4 ────────────────────────────────────────────────────────────
    test('test_zero_distance_fuel_burn: Заправка при дельте 0 км (стоянка с генератором)', () {
      final entries = [
        _makeFuelEntry(id: 1, odometer: 1000.0, volume: 45.0, isFullTank: true),
      ...
      expect(results[1].consumption, isNull,
          reason: 'Дельта 0 км не должна приводить к делению на 0, Infinity или NaN');
      expect(results[1].consumption?.isNaN ?? false, isFalse);
      expect(results[1].consumption?.isInfinite ?? false, isFalse);
    });

    // ── Сценарий 5 ────────────────────────────────────────────────────────────
    test('test_cost_per_km_integrity: Проверка совокупной стоимости 1 км пути при смешанном цикле', () {
    ...
    test('test_micro_distance_guard: Пробег < 15 км блокирует расчет удельного расхода', () {
      final entries = [
        _makeFuelEntry(id: 1, odometer: 1000.0, volume: 45.0, isFullTank: true),
      ...
      expect(results[1].consumption, isNull,
          reason: 'Дельта 10 км < 15 км должна блокировать расчёт аномального расхода (50 л/100 км)');
    });
    ...
    test('test_chazor_volume_limits_validation: Лимиты 53 л для бака и 22 кВт·ч для батареи Chazor', () {
      ...
      final maxFuel = FuelEntryController.getMaxAllowedVolume('fuel', vehicle: chazor);
      final maxEv = FuelEntryController.getMaxAllowedVolume('charge', vehicle: chazor);

      expect(maxFuel, equals(53.0), reason: 'Номинал 48 л + 10% горловина = 53 л');
      expect(maxEv, equals(22.0), reason: 'Номинал 18.3 кВт·ч + потери = 22 кВт·ч');
      ...
      expect(overEvWarning, equals(AnomalyWarning.volumeTooLarge),
          reason: '25 кВт·ч > 22 кВт·ч — превышение емкости батареи с потерями');
    });

    // ── Сценарий 8: Несколько дозаправок до первой полной ─────────────────────
    test('test_partials_before_first_full: Дозаправки до первого полного бака не крашат расчет', () {
      final entries = [
      ...
      expect(results[0].consumption, isNull);
      expect(results[1].consumption, isNull);
      expect(results[2].consumption, isNull, reason: 'Первый полный бак — это anchor/базовая точка');
    ...
    test('test_tail_partials_after_last_full: Хвостовые дозаправки всегда имеют consumption == null', () {
      final entries = [
        _makeFuelEntry(id: 1, odometer: 1000.0, volume: 45.0, isFullTank: true),
        _makeFuelEntry(id: 2, odometer: 1500.0, volume: 40.0, isFullTank: true),
        _makeFuelEntry(id: 3, odometer: 1700.0, volume: 15.0, isFullTank: false),
        _makeFuelEntry(id: 4, odometer: 1850.0, volume: 10.0, isFullTank: false),
      ];

      final results = FuelEntryController.computeEntriesWithConsumption(entries);

      expect(results[1].consumption, closeTo(8.0, 0.001));
      expect(results[2].consumption, isNull, reason: 'Хвостовая дозаправка — null');
      expect(results[3].consumption, isNull, reason: 'Хвостовая дозаправка — null');
    });

    // ── Сценарий 10: Раздельная сводная статистика для гибрида ────────────────
    test('test_overall_stats_separation: Раздельный средний расход бензина и электричества', () {
      final entries = [
        _makeFuelEntry(id: 1, odometer: 1000.0, volume: 40.0, isFullTank: true),
        _makeChargeEntry(id: 2, odometer: 1000.0, volume: 15.0, isFullTank: true),
        _makeFuelEntry(id: 3, odometer: 1500.0, volume: 30.0, isFullTank: true), // 6.0 L/100km
        _makeChargeEntry(id: 4, odometer: 1100.0, volume: 15.0, isFullTank: true), // 15.0 kWh/100km
      ];

      final computed = FuelEntryController.computeEntriesWithConsumption(entries);
    ...
    test('test_user_defined_tank_capacity: Пользовательский объем бака управляет валидацией', () {
      ...
      final nominal = FuelEntryController.getNominalCapacity('fuel', vehicle: customCar);
      final maxFuel = FuelEntryController.getMaxAllowedVolume('fuel', vehicle: customCar);
    ...
    test('test_multi_brand_presets: Проверка авто-определения емкостей для Lixiang, Geely, Tank, etc.', () {
      ...
      expect(FuelEntryController.getNominalCapacity('fuel', vehicle: lixiang), equals(65.0));
      ...
      expect(FuelEntryController.getNominalCapacity('fuel', vehicle: geelyMonjaro), equals(62.0));
      ...
      expect(FuelEntryController.getNominalCapacity('fuel', vehicle: geelyCoolray), equals(45.0));
      ...
      expect(FuelEntryController.getNominalCapacity('fuel', vehicle: geelyGalaxy), equals(60.0));
      ...
      expect(FuelEntryController.getNominalCapacity('fuel', vehicle: tank500), equals(80.0));
      ...
      expect(FuelEntryController.getNominalCapacity('fuel', vehicle: haval), equals(60.0));
    });
    ...
    test('test_user_capacity_overrides_preset: Заданный пользователем объем бака имеет наивысший приоритет', () {
      ...
      final nominal = FuelEntryController.getNominalCapacity('fuel', vehicle: customMonjaro);
      final maxAllowed = FuelEntryController.getMaxAllowedVolume('fuel', vehicle: customMonjaro);

      expect(nominal, equals(70.0), reason: 'Пользовательский ввод должен перекрывать пресет');
