...
void main() {
  group('Vehicle Model', () {
    group('Serialization', () {
      test(
          'toMap and fromMap should correctly serialize and deserialize all fields',
          () {
    ...
    group('Static Methods: isTankRequiredFor', () {
      ...
      test('requires tank for lpg or cng fuelSubtype regardless of engineType',
          () {
        expect(
            Vehicle.isTankRequiredFor(engineType: 'other', fuelSubtype: 'lpg'),
            isTrue);
        expect(
            Vehicle.isTankRequiredFor(engineType: 'other', fuelSubtype: 'cng'),
            isTrue);
      });
    ...
    group('Static Methods: isBatteryRequiredFor', () {
      ...
      test('requires battery for specific hybrid types (PHEV, BEV_REX)', () {
        expect(
            Vehicle.isBatteryRequiredFor(
                engineType: 'hybrid', hybridType: 'PHEV'),
            isTrue);
        expect(
            Vehicle.isBatteryRequiredFor(
                engineType: 'hybrid', hybridType: 'BEV_REX'),
            isTrue);
      });
      ...
      test('does not require battery for other hybrid types or gas/diesel', () {
        expect(
            Vehicle.isBatteryRequiredFor(
                engineType: 'hybrid', hybridType: 'HEV'),
            isFalse);
        expect(
            Vehicle.isBatteryRequiredFor(
                engineType: 'hybrid', hybridType: 'MHEV'),
            isFalse);
    ...
    group(
        'Computed Getters (canCharge, canRefuel, isFullyElectric, isPhev, isSelfChargingHybrid)',
        () {
      test('electric vehicle capabilities', () {
        const ev =
            Vehicle(name: 'EV', model: 'Model 3', engineType: 'electric');
    ...
    group(
        'Data Capacity Helpers (hasBatteryData, hasTankData, effectiveCapacityKwh)',
        () {
      test('hasBatteryData checks', () {
        expect(
            const Vehicle(name: 'Test', model: 'Test').hasBatteryData, isFalse);
        expect(
            const Vehicle(name: 'Test', model: 'Test', batteryCapacityKwh: 10.0)
                .hasBatteryData,
            isTrue);
        expect(
            const Vehicle(name: 'Test', model: 'Test', usableCapacityKwh: 8.0)
                .hasBatteryData,
            isTrue);
        expect(
            const Vehicle(name: 'Test', model: 'Test', batteryCapacityKwh: 0.0)
                .hasBatteryData,
            isFalse);
      });
      ...
      test('hasTankData checks', () {
        ...
        expect(
            const Vehicle(name: 'Test', model: 'Test', tankCapacity: 45.0)
                .hasTankData,
            isTrue);
        expect(
            const Vehicle(name: 'Test', model: 'Test', tankCapacity: 0.0)
                .hasTankData,
            isFalse);
      });
      ...
      test('effectiveCapacityKwh prefers usableCapacityKwh', () {
        const v1 = Vehicle(
            ...
            usableCapacityKwh: 18.0);
        expect(v1.effectiveCapacityKwh, 18.0);

        const v2 =
            Vehicle(name: 'Test', model: 'Test', batteryCapacityKwh: 20.0);
    ...
    group('validateCapacities', () {
      test('returns error if tank required but missing', () {
        const v = Vehicle(
            name: 'Test', model: 'Test', engineType: 'gas', tankCapacity: null);
        expect(v.validateCapacities(),
            contains('Объем топливного бака обязателен'));

        const vZero = Vehicle(
            name: 'Test', model: 'Test', engineType: 'gas', tankCapacity: 0.0);
        expect(vZero.validateCapacities(),
            contains('Объем топливного бака обязателен'));
      });
      ...
      test('returns error if battery required but missing', () {
        ...
        expect(v.validateCapacities(),
            contains('Емкость аккумулятора обязательна'));
      ...
      test('returns null if valid', () {
        const gas = Vehicle(
            name: 'Test', model: 'Test', engineType: 'gas', tankCapacity: 50.0);
    ...
    group('copyWith', () {
      test('updates fields correctly', () {
        ...
        final v2 =
            v1.copyWith(name: 'V2', tankCapacity: 50.0, engineType: 'diesel');
      ...
      test('clears fields correctly', () {
        const v1 = Vehicle(
            name: 'V1', model: 'M1', hybridType: 'PHEV', tankCapacity: 40.0);
